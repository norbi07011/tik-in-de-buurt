...
const BusinessRegistrationForm: React.FC<{ onBack: () => void }> = ({ onBack }) => {
    ...
    const handleFinalSubmit = async (e: React.FormEvent) => {
        ...
        setError(null);
        try {
            const { user, token, business } = await api.registerBusiness(formData);
            loginAction(user, token);
            showToast(t('business_registration_success') || 'Business registered successfully', 'success');
            navigate(Page.RegistrationSuccess, business.id);
        } catch (err: any) {
...
const AuthPage: React.FC = () => {
    ...
    return (
        <div className="auth-container">
            ...
            <div className={`business-reg-wrapper ${isBusinessRegOpen ? 'open' : ''}`}>
                <BusinessRegistrationForm onBack={() => setIsBusinessRegOpen(false)} />
            </div>

            <div className="business-reg-banner" onClick={() => setIsBusinessRegOpen(true)}>
                <p>{t('are_you_a_business_owner')} <span className="font-semibold text-blue-300 hover:underline">{t('register_your_business_here')}</span></p>
            </div>
